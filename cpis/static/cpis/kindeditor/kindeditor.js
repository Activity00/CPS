...
(function (b, c) {
    ...
    a.flash = {
        url: "视频通用地址(支持手机与电脑播放，推荐腾讯视频支持https)",
        width: "宽度",
        height: "高度",
        autostart: "自动播放（只有腾讯视频PC端支持）"
    };
    a.media = {
        url: "媒体文件地址",
        width: "宽度",
...
(function (a, d) {
        ...
        click: function (k) {
            ...
            a.each(i, function (m, o) {
                ...
                g.add(l, function () {
                    ...
                    height: (f[m] + 12) + "px",
                    checked: (h === m.toLowerCase() || h === o.toLowerCase())
                });
            });
            g.show();
            this.menu = g;
    ...
    a.plugin.flash = {
        init: function (f) {
            ...
            var i = a.util.getIframeDoc(this.dialog.iframe);
            var e = a.$("url", i).value;
            ...
            if (!this.check(f, e, g, l)) {
                return false;
            }
            e = e.replace("http://", "//").replace("https://", "//");
            var h = '<iframe frameborder="0" width="640" video="1" height="498" src="' + e + '" allowfullscreen></iframe>';
            a.util.insertHtml(f, h);
            this.dialog.hide();
            a.util.focus(f);
        }
    };
    ...
    a.plugin.image = {
        getSelectedNode: function (i) {
            ...
            this.dialog = new a.dialog({
                id: f,
                cmd: "image",
                file: "image/fckuploadimg.aspx?id=" + f + "&upt=" + a.g[f].config.UploadPathType + "&design=" + a.g[f].config.DesignPage + e,
                width: 620,
                height: 480,
                loadingMode: false,
                title: a.lang.image
    ...
    a.plugin.link = {
        getSelectedNode: function (e) {
            ...
            if (n || a.util.isEmpty(p)) {
                ...
                var q = "http://r.sojump.net.cn/redirect.aspx?url=" + encodeURIComponent(f) + "&activity=" + u;
        ...
        getSelectedTable: function (e) {
            ...
            var f = new a.dialog({
                ...
                yesClickFn: function (v) {
                    var w = a.util.getIframeDoc(f.iframe),
                        m = a.$("width", w),
                        B = a.$("height", w),
                        s = a.$("widthType", w),
                        r = a.$("heightType", w),
            ...
            var j = this.getSelectedTable(m),
                f = this.getSelectedCell(m),
                g = f.cellIndex + l;
            ...
            var j = this.getSelectedTable(m),
                k = this.getSelectedRow(m),
                g = j.insertRow(k.rowIndex + l);
            ...
            var j = this.getSelectedTable(k),
                f = this.getSelectedCell(k),
                g = f.cellIndex;
            for (var h = 0, e = j.rows.length; h < e; h++) {
            ...
            var t = "ke-zeroborder",
                B = a.util.getIframeDoc(this.dialog.iframe),
                x = a.$("mode", B),
                g = a.$("rows", B),
                o = a.$("cols", B),
                l = a.$("width", B),
